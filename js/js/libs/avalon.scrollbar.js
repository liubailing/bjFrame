...
define(function(require) {
    ...
    if (sysConfig.jsTracking) avalon.log('seajs-Tracking："scrollbar---滚动条组件"'); //
    require("A-scrollbar-css");
    ...
    var widget = avalon.ui.scrollbar = function(element, data, vmodels) {
        var options = data.scrollbarOptions
            //方便用户对原始模板进行修改,提高定制性
        var template = require("text!../libs/model/avalon.scrollbar.html")
        ...
        var vmodel = avalon.define(data.scrollbarId, function(vm) {
            ...
            vm.$init = function(continueScan) {
                ...
                avalon.each(children, function(i, item) {
                    ...
                    // 竖直方向支持滚轮事件
                if (vmodel.position.match(/left|right|bottom/g)) {
                    var vs = [],
                        hs = []
                    avalon.each(vmodel._position, function(i, item) {
                        if (item.match(/left|right/g)) {
                            vs.push([i, item])
                        } else if (item.match(/top|bottom/g)){
                            hs.push([i, item])
                        }
                    })
                    ...
                    function wheelLike(diretion, arr, e, func) {
                        avalon.each(arr, function(i, item) {
                            ...
                            vmodel._computer(func || function(obj) {
                                ...
                                //if(!breakOut) e.preventDefault()   //@@liubl
                                e.preventDefault() //@@liubl
                    ...
                    function myOnWheel(e) {
                        ...
                        if (vmodel.inFocuse && vmodel.xinFocuse) {
                             wheelLike(e.wheelDelta > 0 ? "up" : "down", hs, e)
                        }
                    }
            ...
            vm._computer = function(axisComputer, barIndex, position, callback, breakOutCallbackCannotIgnore) {
                ...
                if (bar && bar.data("oni-scrollbar-needed")) {
                    ...
                    if (isVertical) {
                       ...
                       if( xy.y === 0 || xy.y === (obj.draggerparHeight - obj.draggerHeight))
                        {
                            vmodel.xinFocuse = true;
                        }else{
                            vmodel.xinFocuse = false;
                        }
