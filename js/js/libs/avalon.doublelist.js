    ...
    var widget = avalon.ui.doublelist = function(element, data, vmodels) {
        ...
        var vmodel = avalon.define(data.doublelistId, function(vm) {
            ...
            vm._updateAll = function($event, addOrDelete) {
                vmodel.select = []
                vmodel._selectData = []
                vmodel.dataTmpSelect = []
                vmodel.selectTmpSelect = []
                if(addOrDelete === "delete") {
                    vmodel.selectTmpSelect=[]
                    vmodel.dataTmpSelect=[];
                    vmodel._selectData = []
                    vmodel.onChange && vmodel.onChange(1, 0, vmodel)
                }else{
                    vmodel._selectData = vmodel.data;
                    for(var i = 0, len = vmodel.data.length; i < len; i++) {
                         vmodel.select.push(vmodel.data[i].value)
                    }

                    for(var i = 0, len = vmodel.data.length; i < len; i++) {
                          
                            var ele = avalon(document.getElementById("data"+(i+1)+vmodel.$uid))
                            // 重置样式
                            ele.removeClass("oni-state-active").addClass("oni-state-disabled")
                            if(vmodel.hideSelect) ele.addClass("oni-helper-hidden")                         
                    }
                ...
                //     vmodel.dataTmpSelect.push(vm.data[i].value)
                // }
                // for (var i = 0; i < vm.select.length; i++) {
                //     vmodel.selectTmpSelect.push(vm.select[i].value)
                // }
                // var tar = addOrDelete === "delete" ? vmodel.selectTmpSelect : vmodel.dataTmpSelect
                // if(tar.length == 0) return
                // if(!vmodel.countLimit(vmodel.select, addOrDelete, tar.length)) return
