...
//define(["avalon", "./avalon.tree", "text!./avalon.tree.check.html"], function(avalon, tree, check_html) {
    ...
    if (sysConfig.jsTracking) avalon.log('seajs-Tracking："tree.check --- 选择树"'); //
    var  check_html = require("text!../libs/model/avalon.tree.check.html");
			...
			avalon.mix(vm, {
				...
				checkNode: function(leaf, checked, checkTypeFlag, callbackFlag) {
					...
					vm.excute("checkChange", {
						...
						leaf.checked = chk
						callbackFlag && onCheck && onCheck(arg)  //callbackFlag && chk && onCheck && onCheck(arg)
						return chk
					})
				},
				/**
	             ...
	             * @param {Object} 可以指定一个起始的节点
	             */
				getChkIds: function(leaf) {
					var checkedNodes = vm.getCheckedNodes(true,leaf)
