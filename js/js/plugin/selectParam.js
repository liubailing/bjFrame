...
(function ($) {
    ...
    var methods = {
        init: function (opt) {
            ...
            if (this.opt.datasource.length > 0) {
                for (var i = 0; i < this.opt.datasource.length; i++) {
                    $obj.selectParam("setChecked", this.opt.datasource[i].name);
                }//end循环
            }
            return $obj;
        },
        ...
        delChoise: function (name) {
            ...
            var $obj = $(this);
            $obj.selectParam("delChecked", name);
            var $checkitems = $obj.find("div.cc-checkitem");
        ...
        replaceChoise: function (optitem) {
            ...
            var $obj = $(this);
            $obj.selectParam("delChecked", optitem.name);
            var $checkitems = $obj.find("div.cc-checkitem");
            ...
            if (set.initData && set.initData.length > 0) {
                $obj.selectParam("setChecked", set.name);
            }
        },
        ...
        resetChoise: function (optitem, afterName) {
            ...
            if (optitem.initData && optitem.initData.length > 0) {
                $obj.selectParam("setChecked", optitem.name);
            }
        }
    };
    // 已选项目
    ...
    var ccChoiceDom = {
        doTime: function ($objchk) {
            ...
            stime[0] = stime[1] == "" ? stime[0] : (stime[1] + "-" + stime[1]);
            ccChoiceDom.initDom(
                $objchk,
                {
                    name: tilte,
    ...
    $.fn.selectParam = function (method) {
